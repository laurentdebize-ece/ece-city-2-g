//
// Created by lulub on 19/10/2022.
//
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#ifndef UNTITLED2_GRAPHE_H
#define UNTITLED2_GRAPHE_H
...
int rechercheRoute(int i, int j, int tab[45][35], int distance)
{
    if (tab[i - 1][j] == 6)
    {
        ...
        i = i - 1;
        j = j;
        distance = rechercheRoute(i, j, tab,distance);
    }
    if (tab[i + 1][j] == 6)
    {
        distance++;
...
Graphe* conversionTXTgraphe() {
    FILE *ifs = fopen("matriceCase.txt","r");
    Graphe* graphe;
    int tab[45][35];
    int distance,u,v;
    int ordre=0;
    ...
    for (int i = 0; i < 35; i++) //récupérer les données de la matrice
    {
        for (int j = 0; j < 45; j++)
        {
                    ...
                    case 2: //cabane 10H  3*3
                        for (int k = 0; k < 3; k++)
                        {
                            for (int l = 0; l < 3; l++)
                            {
                                ...
                                if (tab[i+k-u][j + l] == 6)
                                {
                                    distance=rechercheRoute(i+k-u,j+l,tab,distance);
                                    printf("distance");
                                    u++;
                                }
                                if (tab[i+k+u][j + l] == 6)
